...
function Server(opts){
  ...
  if (opts.session_store && opts.session_store.host && opts.session_store.port) {
    redisClient = redis.createClient(opts.session_store.port, opts.session_store.host);
  } else {
    this.sessions = {};
  }
...
Server.prototype.client = function (sid, value, fn) {
  ...
  if (redisClient === undefined) {
    ...
    if ("function" === typeof value || (json = JSON.stringify(value)) === undefined)
      redisClient.hget(["engine.io", sid], function (err, val) {
        if (!err && "function" === typeof value) value(JSON.parse(val));
      });
    else if (value === null)
      redisClient.hdel(["engine.io", sid]);
    else
      redisClient.hset(["engine.io", sid, json]);
  }
};
...
Server.prototype.attach = function(server, opts) {
  var self = this;
  var options = opts || {};
